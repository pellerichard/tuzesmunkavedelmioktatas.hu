<template>
    <div>
        <v-layout row justify-center>
            ...
            >
                <v-card>
                    <v-card-title class="headline">Cég szerkesztése</v-card-title>
                    <v-card-text>
                        <!-- Inputok -->
                        <v-layout row>
                            <v-flex xs6>
                                <v-text-field prepend-icon="perm_contact_calendar" v-model="company.name" class="form-control" id="name" placeholder="* Cég neve" type="text"></v-text-field>
                            </v-flex>
                            <v-flex xs6>
                                <v-select
                    ...
                    <v-card-actions class="inline-block">
                        ...
                        <button type="button" class="btn btn-success" @click="editCompany()" data-dismiss="modal">Cég mentése</button>
                    </v-card-actions>
                </v-card>
            </v-dialog>
        </v-layout>
...
<script>
    export default {
        ...
        methods: {
            editCompany() {
                this.$store.dispatch('company/editCompany', {
